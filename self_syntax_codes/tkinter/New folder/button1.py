...
def install_sql(s_name,u_name,passwd):
	...
	window2.title("Mphasis Automation App")
	window2.geometry("440x410")
	a=r"C:\MdbBOT\SQL\scripts\a.PS1"
	cmd='powershell.exe -noexit -file "{0}" "{1}" "{2}" "{3}"'.format(a,s_name,u_name,passwd)
	print cmd
	t_start="SQL DB Installation started at {0}".format(time.asctime(time.localtime()))
	tk.Label(window2,text=t_start).pack()
	...
	rc=p.returncode
	if (rc==0):
		tk.Label(window2,text="SQL DB has been successfully installed.").pack()
		t_stop="SQL DB Installation finished at {0}".format(time.asctime(time.localtime()))
		tk.Label(window2,text=t_stop).pack()
	else:
		tk.Label(window2,text="SQL DB installation has failed.").pack()

def patch_sql(s_name,u_name,passwd):
	window2=tk.Tk()
	window2.title("Mphasis Automation App")
	window2.geometry("440x410")
	a=r"C:\MdbBOT\SQL\scripts\b.PS1"
	cmd='powershell.exe -noexit -file "{0}" "{1}" "{2}" "{3}"'.format(a,s_name,u_name,passwd)
	print cmd
	t_start="SQL DB Patching started at {0}".format(time.asctime(time.localtime()))
	tk.Label(window2,text=t_start).pack()
	p = Popen(cmd, shell=True)
	stdout, stderr = p.communicate()
	#print stdout.readlines()
	#print stderr.readlines()
	tk.Label(window2,text="SQL DB has been successfully patched.").grid()
	t_stop="SQL DB Patching finished at {0}".format(time.asctime(time.localtime()))
	tk.Label(window2,text=t_stop).pack()
...
def create_sql(s_name,u_name,passwd,i_name,db_name):
	...
	window2.title("Mphasis Automation App")
	window2.geometry("440x410")
	a=r"C:\MdbBOT\SQL\scripts\c.PS1"
	cmd='powershell.exe -noexit -file "{0}" "{1}" "{2}" "{3}" "{4}" "{5}"'.format(a,s_name,u_name,passwd,i_name,db_name)
	print cmd
	t_start="SQL DB Creation started at {0}".format(time.asctime(time.localtime()))
	tk.Label(window2,text=t_start).pack()
	p = Popen(cmd, shell=True)
	stdout, stderr = p.communicate()
	#print stdout.readlines()
	#print stderr.readlines()	
	tk.Label(window2,text="SQL DB has been successfully created.").grid()
	t_stop="SQL DB Creation finished at {0}".format(time.asctime(time.localtime()))
	tk.Label(window2,text=t_stop).pack()
...
def hello_world3():
	def install_form3():
		...
		tk.Label(frame7,text="").grid(sticky="W")
		e1=tk.Entry(frame7)
		e1.grid(row=1,column=1,sticky="W")
		e2=tk.Entry(frame7)
		e2.grid(row=2,column=1,sticky="W")
		...
		e3.grid(row=3,column=1,sticky="W")
		framet=tk.Frame(window1)
		framet.pack()
		tk.Button(framet, activeforeground="#000fff000", bg="lightgrey", text="Install SQL", padx=10, pady=3, command= lambda: install_sql(e1.get(),e2.get(),e3.get())).pack()
	window1=tk.Tk()
	window1.title("Mphasis Automation App")
	window1.grid()
	window1.geometry("440x410")
	#frame3=tk.Frame(window1,bd=5)
	label=tk.Label(window1,text='Please upload the request form(ini) file in "C:\\MdbBOT\\SQL\\config\\install folder"\n\n And click on Proceed to continue\n\n')
	label.pack()
...
def hello_world4():
	def install_form4():
		...
		tk.Label(frame8,text="").grid(sticky="W")
		e1=tk.Entry(frame8)
		e1.grid(row=1,column=1,sticky="W")
		e2=tk.Entry(frame8)
		e2.grid(row=2,column=1,sticky="W")
		...
		e3.grid(row=3,column=1,sticky="W")
		frameu=tk.Frame(window1)
		frameu.pack()
		tk.Button(frameu, activeforeground="#000fff000", bg="lightgrey", text="Patch SQL", padx=10, pady=3, command= lambda: patch_sql(e1.get(),e2.get(),e3.get())).pack()
	window1=tk.Tk()
	window1.title("Mphasis Automation App")
	window1.grid()
	window1.geometry("440x410")
	#frame3=tk.Frame(window1,bd=5)
	label=tk.Label(window1,text='Please upload the request form(ini) file in "C:\\MdbBOT\\SQL\\config\\patch folder"\n\n And click on Proceed to continue\n\n')
	label.pack()
...
def hello_world5():
	def install_form5():
		...
		tk.Label(frame9,text="").grid(sticky="W")
		e1=tk.Entry(frame9)
		e1.grid(row=1,column=1,sticky="W")
		e2=tk.Entry(frame9)
		e2.grid(row=2,column=1,sticky="W")
		...
		e5=tk.Entry(frame9)
		e5.grid(row=5,column=1,sticky="W")
		framev=tk.Frame(window1)
		framev.pack()
		tk.Button(framev, activeforeground="#000fff000", bg="lightgrey", text="Create DB", padx=10, pady=3, command= lambda: create_sql(e1.get(),e2.get(),e3.get(),e4.get(),e5.get())).pack()
	window1=tk.Tk()
	window1.title("Mphasis Automation App")
	window1.grid()
	window1.geometry("440x410")
	#frame3=tk.Frame(window1,bd=5)
	label=tk.Label(window1,text='Please upload the request form(ini) file in "C:\\MdbBOT\\SQL\\config\\db_create folder"\n\n And click on Proceed to continue\n\n')
	label.pack()
...
ttk.Label(frameb,text="").pack()
ttk.Label(frameb,text="").pack()
frame2=tk.LabelFrame(frameb,cursor="top_left_corner",text="Choose an option",padx=20,pady=20,bd=5)
frame2.pack(fill="y")
ttk.Label(frameb,text="").pack()
ttk.Label(frameb,text="").pack()
