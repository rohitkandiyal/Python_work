...
def install_sql(s_name,u_name,passwd):
	...
	#print stderr.readlines()
	#p.terminate()
	tk.Label(window2,text="SQL DB has been successfully installed.\n\n Please check the logs in \n C:\\Program Files\\Microsoft SQL Server\\100\\Setup Bootstrap\\Log\\Summary.txt").grid()
...
def create_sql(s_name,u_name,passwd):
	window2=tk.Tk()
	window2.title("Mphasis Automation App")
	window2.geometry("440x410")
	a=r"C:\MdbBOT\SQL\scripts\c.PS1"
	file=sr_home+"config\\db_create\\db_create.txt"
	txt = open(file)
	args=[]
	for line in txt.readlines():
		args.append(line.split(",")[1])
	...
	#print stdout.readlines()
	#print stderr.readlines()	
	tk.Label(window2,text="SQL DB has been successfully created.").pack()
...
def hello_world5():
	def install_form5():
		...
		framev.pack()
		tk.Button(framev, activeforeground="#000fff000", bg="lightgrey", text="Create DB", padx=10, pady=3, command= lambda: create_sql(e1.get(),e2.get(),e3.get())).pack()
	window1=tk.Tk()
	window1.title("Mphasis Automation App")
	window1.grid()
	window1.geometry("450x410")
	#frame3=tk.Frame(window1,bd=5)
